...
class admin_model extends CI_Model
{
    ...
    {
        $this->db->query("UPDATE [dbo].[Peminjaman] SET Approval = 1 WHERE ID_Peminjaman='$id'");
    }
    ...
    {
        $data = $this->db->query("SELECT [dbo].[Peminjaman].ID_Peminjaman, [dbo].[User].username, [dbo].[Book].Judul,[dbo].[Peminjaman].Tgl_Peminjaman,[dbo].[Peminjaman].Tgl_Pengembalian,[dbo].[Peminjaman].Notifikasi, [dbo].[Peminjaman].Approval FROM dbo.Peminjaman INNER JOIN [dbo].[User] ON [dbo].[Peminjaman].id = [dbo].[User].id INNER JOIN [dbo].[Book] ON [dbo].[Peminjaman].ID_Buku = [dbo].[Book].ID");
        return $data->result_array();
    }
